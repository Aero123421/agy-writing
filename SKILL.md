...
2. Confirm `agy` is installed and authenticated. When the exact model is not given, run `agy models` and prefer an available model that clearly corresponds to Gemini 3.8 Flash; copy the available model ID verbatim and never guess its exact ID. If Gemini 3.8 Flash is not available, use another suitable available Gemini model rather than treating 3.8 Flash as a hard requirement.
